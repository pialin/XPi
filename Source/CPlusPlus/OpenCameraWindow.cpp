								...
								//ArgCount代表调用此函数时输入参数的个数;ArgVector是输入参数字符串数组的头指针
{
    ...
    //const cv::Rect ButtonArea = cv::Rect(
    //                                    FrameWidth*0.8,FrameHeight*0.8，
    //                                    FrameWidth*0.1,FrameHeight*0.1
    //                                );                          //设置按钮的位置和尺寸                                                                                
    ...
    signed char PressedKeyAscii;                                       //用于存储按下按键的Ascii码
	...
	if(CameraFrame.empty()) break;				//判断是否成功取到图像帧,如果无法取到帧则跳出循环，退出函数
        ...
        //CameraFrame(ButtonArea) = CameraFrame(ButtonArea) *
	...
	if(PressedKeyAscii == 32) 	                        //在此判断返回值大于等于0目的是对任意键响应，即按任意键结束摄像头
        {   
            ...
            cv::waitKey(3000); 
            ResultText = "Hello OpenCV!";                    //TODO 加上tensorflow代码
            ...
            cv::imshow("Camera",CameraFrame);                   //绘制文字后更新窗口显示内容  
